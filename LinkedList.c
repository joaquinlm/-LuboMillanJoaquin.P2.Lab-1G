...
static int addNode(LinkedList *this, int nodeIndex, void *pElement)
{
  ...
  if (this != NULL && nodeIndex >= 0 && nodeIndex <= ll_len(this))
  {
    ...
    if (newNode != NULL)
    {
      ...
      if (nodeIndex == 0)
      {
        newNode->pNextNode = this->pFirstNode; // apunta al cero actual
        this->pFirstNode = newNode;
      }
      else
      {
        auxPreviousNode = getNode(this, nodeIndex - 1);
        newNode->pNextNode = auxPreviousNode->pNextNode; // apunta al cero actual
...
int ll_remove(LinkedList *this, int index)
{
  ...
  if (this != NULL && index >= 0 && index < ll_len(this))
  {
    ...
    else
    {
      ...
      {
        auxPreviousNode->pNextNode = auxPreviousNode->pNextNode->pNextNode;
      }
    }
    this->size = this->size - 1;
    returnAux = 0;
...
int ll_indexOf(LinkedList *this, void *pElement)
{
  ...
  if (this != NULL)
  {
    int tam = ll_len(this); // para no llamar a ll_len por cada iteracion
    for (int i = 0; i < tam; i++)
    {
